...
## Day 3–4 (Aug 8–9) — Teacher training — DONE, Gate 2 PASSED

**Goal:** `teacher_final.pt` passes Gate 2. **Achieved**, after two real bugs found and fixed:

- [x] First real run: Gate 2 **failed** (`QWK_dual=0.558 < QWK_macula=0.572`). Root cause: the
  fusion mechanism (concat → single Linear + `BatchNorm1d`) was architecturally too weak to model
  interaction between views (`judge.md` Flag 2, now confirmed empirically) and batch_size=8
  `BatchNorm1d` statistics were plausibly adding noise on top. **Fixed**: `InteractionFusion`
  (adds `|z_m − z_d|` and `z_m ⊙ z_d` terms through a small MLP, uses `LayerNorm` instead) +
  teacher batch_size bumped 8→16.
- [x] Second real run (with the fix): hit a **Google Drive write/read race** — `train_teacher()`
  saved the freeze-phase checkpoint and crashed on the very next line trying to read it back
  (`FileNotFoundError`), because Drive's FUSE mount can lag a beat behind its own writes.
  **Fixed**: the freeze→finetune handoff now uses an in-memory copy of the best state dict
  instead of a disk round-trip, and a new `robust_torch_load()` helper (retry + backoff) covers
  every other checkpoint read in the notebook against the same class of race.
- [x] Third run: **Gate 2 PASSED.** `QWK_dual=0.596 > max(QWK_macula=0.567, QWK_disc=0.559)`,
  `G_internal=+0.0295`. `teacher_final.pt` on Drive is the final, valid teacher checkpoint —
  no need to re-run this section again.

Full root-cause writeup for both bugs: `experiment/documentation.md` Sections 0 and 0.1.
